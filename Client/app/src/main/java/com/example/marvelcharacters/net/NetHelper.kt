...
class NetHelper {
    ...
    fun getCharacters(): Observable<CharacterResponse> {
        ...
        val interfaceObj = requestInterface.create(GetHeroes::class.java)
    ...
    fun getComics(): Observable<ComicsResponse> {
        ...
        // hash - a md5 digest of the ts parameter, your private key and your public key (e.g. md5(ts+privateKey+publicKey)
        val hash = "$ts$privateKey$publicKey".md5()
        val interfaceObj = requestInterface.create(GetComics::class.java)
        return interfaceObj.getData(publicKey, ts, hash)
    }

    fun getCreators(): Observable<CreatorsResponse> {
        // ts - a timestamp (or other long string which can change on a request-by-request basis)
        val ts = 1

        // hash - a md5 digest of the ts parameter, your private key and your public key (e.g. md5(ts+privateKey+publicKey)
        val hash = "$ts$privateKey$publicKey".md5()
        val interfaceObj = requestInterface.create(GetCreators::class.java)
        return interfaceObj.getData(publicKey, ts, hash)
    }
}